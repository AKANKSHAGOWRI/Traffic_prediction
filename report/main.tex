...
\chapter*{Abstract}
This dissertation develops a predictive analytics system for traffic congestion in London using Department for Transport (DfT) traffic count data spanning 2006 to 2019. The system implements a silver-gold data lake architecture orchestrated through Apache Airflow, processing hourly traffic volumes across London's road network. Raw CSV data is cleaned and converted to partitioned Parquet format, then enriched with engineered features. Feature engineering incorporates temporal patterns (hour of day, day of week, month, year, weekend indicators), lag features (1-hour, 2-hour, 24-hour), and rolling statistics (24-hour mean and standard deviation). Two machine learning models are compared: Linear Regression and Gradient Boosted Trees, evaluated using walk-forward validation with expanding training windows. The Gradient Boosted Trees model achieves superior performance with R² scores of 0.88-0.90, RMSE of 237-355 vehicles per hour, and MAE of 150-194 across validation folds. Hotspot identification uses percentile-based thresholds to classify high-congestion road segments. A weighted routing graph demonstrates congestion-aware path planning, showing route time reductions of 15-25\% compared to distance-only routing during peak hours. Outputs are visualized through dual dashboards: a web-based Mapbox interface hosted on GitHub Pages at \url{https://narendrachit.github.io/Traffic_prediction/} and a desktop Power BI dashboard. The system provides actionable insights for urban traffic management and demonstrates the effectiveness of tree-based models for temporal traffic prediction.
...
\include{chapters/03_data_and_problem}
\include{chapters/04_methodology}
\include{chapters/05_system_design}
\include{chapters/06_results_evaluation}
