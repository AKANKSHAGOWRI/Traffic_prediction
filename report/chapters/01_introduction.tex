...
Four research questions structure the work. RQ1 (Forecasting): How accurately can congestion hotspots in London be forecast using Department for Transport historical traffic count data with temporal and spatial features? RQ2 (Patterns): What spatio-temporal patterns characterise congestion in London, including peak versus off-peak periods, weekday versus weekend differences, seasonality, and spatial clustering? RQ3 (Model comparison): Which modelling approach is most reliable under walk-forward validation, comparing linear regression and tree-based methods, and why? RQ4 (Decision support): How effectively can predicted congestion risk be communicated via dual interactive dashboards with maps, hotspot visualization, and congestion-aware routing to support operational decisions? These questions ensure that the dissertation addresses data engineering, predictive performance, spatial interpretation, and practical delivery.

The first question emphasizes predictive accuracy using DfT traffic counts as the primary data source, with feature engineering to capture temporal patterns and spatial relationships. The second question focuses on temporal and spatial structure that shapes congestion dynamics across London's road network. The third question evaluates model reliability under realistic temporal validation, comparing interpretable linear models with gradient boosted trees to understand trade-offs between simplicity and performance. The fourth question connects analytics to communication and usability through both web-based and desktop visualization platforms, which is essential for applied decision support in transport planning contexts.
...
\midrule
RQ1 & DfT traffic counts (2006-2019) & Data lake architecture, feature engineering, hotspot forecasting & Fig.~\ref{fig:pipeline-concept}, Tab.~\ref{tab:dictionary} & Ch.3 Data and Problem; Ch.4 Methodology \\
RQ2 & DfT hourly counts; spatial network & Temporal profiling, spatial clustering, heatmap analysis & Fig.~\ref{fig:heatmap}, Fig.~\ref{fig:seasonal}, Fig.~\ref{fig:hotspot-map} & Ch.6 Results and Evaluation \\
RQ3 & Engineered features; walk-forward folds & Linear Regression vs Gradient Boosted Trees & Tab.~\ref{tab:model-compare}, Fig.~\ref{fig:error-peak} & Ch.6 Results and Evaluation \\
RQ4 & GeoJSON routes and hotspots; metrics CSV & Mapbox and Power BI dashboards, route comparison & Fig.~\ref{fig:dash-views}, Fig.~\ref{fig:architecture} & Ch.5 System Design; Ch.6 Results \\
\bottomrule
\end{tabular}
